...
export default function Lines({ guess, isFinal, solution, isEmpty }: guessType) {
  ...
  for (let i = 0; i < WORD_LENGTH; i++) {
    const char = guess[i]
    if (isFinal) {
      if (char === solution[i]) { status += 'bg-green-500 ' }
      else if (solution.includes(char)) { status += 'bg-yellow-500 ' }
      else if (char !== solution[i]) { status += 'bg-red-300 ' }
    }
    tiles.push(
      <div key={i}
  ...
  return <div className={`${empty} flex items-center justify-center gap-1`}> {tiles}</div>
}
